...
ABBREVIATIONS: dict[str, str] = json.load(open('./assets/address_abbreviations.json'))
...
# Name
PROPORTION_SWAPPED_NAMES = 0.5 # Swap FIRST and LAST name
PROPORTION_TWINS = 0.5 # Alter FIRST and MRN
PROPORTION_PLACEHOLDER = 0 # FIRST and LAST -> Jane/John/Baby Doe/Girl/Boy
PROPORTION_COMMONLY_SHORT_NAMES = 0 # Alter FIRST to a short name from a list of short names
PROPORTION_COMPOUND_NAMES = 0
PROPORTION_UNKOWN = 0
PROPORTION_HYPHENATED = 0 # Add a hyphen and add another name
...
def reverse_date(date_str):
    try:
        ...
        reversed_dob.strftime('%Y-%m-%d')  # this will raise ValueError if invalid
...
def scramble_data(
    ...
    good_data = source_data_with_copies.sample(frac=PROPORTION_NO_ERRORS, random_state=seed)
    ...
    # Twins
    twins = source_data_with_copies.sample(frac=PROPORTION_TWINS, random_state=seed)
    ...
    swap_first_last = source_data_with_copies.sample(frac=PROPORTION_SWAPPED_NAMES, random_state=seed)
    ...
    hyphenated_last_names = source_data_with_copies.sample(frac=PROPORTION_HYPHENATED, random_state=seed).copy()
    ...
    punctuated_name = source_data_with_copies.sample(frac=PROPORTION_PUNCTUATED, random_state=seed).copy()
    ...
    punctuate_last_name = source_data_with_copies.sample(frac=PROPORTION_PUNCTUATED_LAST, random_state=seed).copy()
    ...
    # Compound Names
    compound_names = source_data_with_copies.sample(frac= PROPORTION_COMPOUND_NAMES, random_state = seed).copy()
    ...
    # Alias
    bad_name_nickname = source_data_with_copies.sample(frac=PROPORTION_NICKNAME, random_state=seed)
    ...
    short_names = source_data_with_copies.sample(frac=PROPORTION_NICKNAME, random_state=seed).copy()
    num_letters = [3, 4]
    ...
    nickname_file = './assets/short-names.csv'
    nickname_dict = load_nicknames(nickname_file)
    commonly_short_names = replace_names_with_nicknames(source_data_with_copies, nickname_dict, PROPORTION_COMMONLY_SHORT_NAMES, seed)
    commonly_short_names['GroundTruth'] = 'Match'
    commonly_short_names['Description'] = "Nicknames"
    commonly_short_names[SCENARIO_NUM] = "8.2"

    # Short Names
    short_names = source_data_with_copies.sample(frac=PROPORTION_SHORT_NAMES, random_state=seed).copy()
    num_letters = [2, 3]
    for index, row in short_names.iterrows():
        i = random.choice(num_letters)
        short_names.at[index, 'FIRST'] = row['FIRST'][:i]
    ...
    place_holder_name = source_data_with_copies.sample(frac=PROPORTION_PLACEHOLDER, random_state=seed).copy()
    ...
    # ---- No Match Logic -----
    place_holder_name_no_match = source_data_with_copies.sample(frac=PROPORTION_PLACEHOLDER_NO_MATCH, random_state=seed).copy()
    place_holder_list = [('Baby', 'Boy'), ('Baby', 'Girl'), ('Jane', 'Doe'), ('John', 'Doe')]
    ...
    place_holder_name_no_match['BIRTHDATE']= faker.date_of_birth(minimum_age=18, maximum_age=90).strftime('%Y-%m-%d')
    ...
    unknown_names = source_data_with_copies.sample(frac=PROPORTION_UNKOWN, random_state=seed).copy()
    ...
    #----- No Match -----
    anon_names_list = ['Unk', 'Anonymous', 'Anon', 'Unknown']
    unknown_names_no_match = source_data_with_copies.sample(frac=PROPORTION_UNKOWN_NO_MATCH, random_state=seed).copy()
    for index, row in unknown_names.iterrows():
        selected_unknown = random.choice(anon_names_list)
    ...
    unknown_names_no_match['BIRTHDATE'] = faker.date_of_birth(minimum_age=18, maximum_age=90).strftime('%Y-%m-%d')
    ...
    filtered_data = source_data_with_copies[source_data_with_copies['GENDER'] == 'F']
    ...
    # -------------Scrambling DOB------------
    source_data_with_copies['BIRTHDATE_DT'] = pd.to_datetime(source_data_with_copies['BIRTHDATE'], format='%m-%d-%Y', errors='coerce')
    valid_dates = source_data_with_copies[source_data_with_copies['BIRTHDATE_DT'].dt.day <= 12]
    ...
    apt_unit = source_data_with_copies.sample(frac=PROPORTION_APT_UNIT, random_state=seed).copy()
    ...
    apt_unit_no_match = source_data_with_copies.sample(frac=PROPORTION_APT_UNIT_NO_MATCH, random_state=seed).copy()
    apt_unit_no_match['ADDRESS'] = apt_unit_no_match['ADDRESS'].apply(update_apartment_number)
    apt_unit_no_match['SSN'] = faker.ssn()
    apt_unit_no_match['BIRTHDATE'] = faker.date_of_birth(minimum_age=18, maximum_age=90).strftime('%Y-%m-%d')
    ...
    congregate_settings = source_data_with_copies.sample(frac=PROPORTION_CONGREGATE_SETTINGS, random_state=seed).copy()
    ...
    congregate_settings_no_match = source_data_with_copies.sample(frac=PROPORTION_CONGREGATE_SETTINGS_NO_MATCH, random_state=seed).copy()
    congregate_settings_no_match['SSN'] = faker.ssn()
    congregate_settings_no_match['BIRTHDATE'] = faker.date_of_birth(minimum_age=18, maximum_age=90).strftime('%Y-%m-%d')
    ...
    upper_lower_address = source_data_with_copies.sample(frac=PROPORTION_UPPER_LOWER_CASE, random_state=seed).copy()
    ...
    # ---- No Match Logic----
    upper_lower_address_no_match = source_data_with_copies.sample(frac=PROPORTION_UPPER_LOWER_CASE_NO_MATCH, random_state=seed).copy()
    ...
    upper_lower_address_no_match['BIRTHDATE'] = faker.date_of_birth(minimum_age=18, maximum_age=90).strftime('%Y-%m-%d')
    ...
    spaced_address = source_data_with_copies.sample(frac=PROPORTION_SPACED, random_state=seed).copy()
    ...
    # ----- No Match Logic --- 
    spaced_address_no_match = source_data_with_copies.sample(frac=PROPORTION_SPACED_NO_MATCH, random_state=seed).copy()
    ...
    spaced_address_no_match['BIRTHDATE'] = faker.date_of_birth(minimum_age=18, maximum_age=90).strftime('%Y-%m-%d')
    ...
    punctuated = source_data_with_copies.sample(frac=PROPORTION_PUNCTUATION, random_state=seed).copy()
    ...
    # --- No Match Logic--- 
    punctuated_no_match = source_data_with_copies.sample(frac=PROPORTION_PUNCTUATION_NO_MATCH, random_state=seed).copy()
    punctuated_no_match['ADDRESS'] = punctuated_no_match['ADDRESS'].apply(insert_punctuation)
    punctuated_no_match['SSN'] = faker.ssn()
    punctuated_no_match['BIRTHDATE'] = faker.date_of_birth(minimum_age=18, maximum_age=90).strftime('%Y-%m-%d')
    ...
    # PO BOX
    po_box_data = source_data_with_copies[source_data_with_copies['ADDRESS'].str.contains('PO BOX', case=False, na=False)].copy()
    po_box = po_box_data.sample(frac=PROPORTION_PO_BOX, random_state=seed).copy()
    ...
    po_box_no_match = po_box_data.sample(frac=PROPORTION_PO_BOX_NO_MATCH, random_state=seed).copy()
    for index, row in po_box_no_match.iterrows():
        po_box_no_match.at[index, 'SSN'] = faker.ssn()
        po_box_no_match.at[index, 'BIRTHDATE'] = faker.date_of_birth(minimum_age=18, maximum_age=90).strftime('%Y-%m-%d')
    ...
    general_delivery_data = source_data_with_copies[source_data_with_copies['ADDRESS'].str.contains('GENERAL DELIVERY', case=False, na=False)].copy()

    general_delivery = general_delivery_data.sample(frac=PROPORTION_GENERAL_DELIVERY, random_state=seed).copy()
    ...
    general_delivery_no_match = general_delivery_data.sample(frac=PROPORTION_GENERAL_DELIVERY_NO_MATCH, random_state=seed).copy()
    for index, row in general_delivery_no_match.iterrows():
        general_delivery_no_match.at[index, 'SSN'] = faker.ssn()
        general_delivery_no_match.at[index, 'BIRTHDATE'] = faker.date_of_birth(minimum_age=18, maximum_age=90).strftime('%Y-%m-%d')
    ...
    emergency_mail_data = source_data_with_copies[source_data_with_copies['ADDRESS'].str.contains('EMERGENCY', case=False, na=False)].copy()
    PROPORTION_EMERGENCY_MAIL_NO_MATCH = PROPORTION_EMERGENCY_MAIL

    emergency_mail = emergency_mail_data.sample(frac=PROPORTION_EMERGENCY_MAIL, random_state=seed).copy()
    ...
    emergency_mail_no_match = emergency_mail_data.sample(frac=PROPORTION_EMERGENCY_MAIL_NO_MATCH, random_state=seed).copy()
    for index, row in emergency_mail_no_match.iterrows():
        emergency_mail_no_match.at[index, 'SSN'] = faker.ssn()
        emergency_mail_no_match.at[index, 'BIRTHDATE'] = faker.date_of_birth(minimum_age=18, maximum_age=90).strftime('%Y-%m-%d')
    ...
    # USPS Store Location
    PROPORTION_USPS_STORE_NO_MATCH = PROPORTION_USPS_STORE
    usps_store_data = source_data_with_copies[source_data_with_copies['ADDRESS'].str.contains('USPS', case=False, na=False)].copy()

    usps_store = usps_store_data.sample(frac=PROPORTION_USPS_STORE, random_state=seed).copy()
    ...
    usps_store_no_match = usps_store_data.sample(frac=PROPORTION_USPS_STORE_NO_MATCH, random_state=seed).copy()
    for index, row in usps_store_no_match.iterrows():
        usps_store_no_match.at[index, 'SSN'] = faker.ssn()
        usps_store_no_match.at[index, 'BIRTHDATE'] = faker.date_of_birth(minimum_age=18, maximum_age=90).strftime('%Y-%m-%d')
    ...
    community_services_data = source_data_with_copies[~source_data_with_copies['ADDRESS'].str.contains('GENERAL|EMERGENCY|PO BOX|USPS', case=False, na=False)].copy()

    community_services = community_services_data.sample(frac=PROPORTION_COMMUNITY_SERVICES, random_state=seed).copy()
    ...
    community_services_no_match = community_services_data.sample(frac=PROPORTION_COMMUNITY_SERVICES_NO_MATCH, random_state=seed).copy()
    community_services_no_match['SSN'] = faker.ssn()
    community_services_no_match['BIRTHDATE'] = faker.date_of_birth(minimum_age=18, maximum_age=90).strftime('%Y-%m-%d')
    ...
    leading_zero = source_data_with_copies.sample(frac=PROPORTION_LEADING_ZERO, random_state=seed).copy()
    ...
    leading_zero_no_match = source_data_with_copies.sample(frac=PROPORTION_LEADING_ZERO_NMO_MATCH, random_state=seed).copy()
    leading_zero_no_match['ADDRESS'] = leading_zero_no_match['ADDRESS'].apply(add_leading_zeros)
    leading_zero_no_match['SSN'] = faker.ssn()
    leading_zero_no_match['BIRTHDATE'] = faker.date_of_birth(minimum_age=18, maximum_age=90).strftime('%Y-%m-%d')
    ...
    filtered_data = source_data_with_copies[source_data_with_copies['ADDRESS'].apply(lambda x: contains_abbreviation(x, ABBREVIATIONS))]
    ...
    abbreviated_no_match['BIRTHDATE'] = faker.date_of_birth(minimum_age=18, maximum_age=90).strftime('%Y-%m-%d')
    ...
    ssn_last_four = source_data_with_copies.sample(frac=PROPORTION_SSN_LAST_FOUR, random_state=seed).copy()
    ...
    ssn_last_four_no_match = source_data_with_copies.sample(frac=PROPORTION_SSN_LAST_FOUR_NO_MATCH, random_state=seed).copy()
    ssn_last_four_no_match['SSN'] = ssn_last_four_no_match['SSN'].apply(retain_last_four_ssn)
    ssn_last_four_no_match['BIRTHDATE'] = faker.date_of_birth(minimum_age=18, maximum_age=90).strftime('%Y-%m-%d')
...
def main():
  ...
  # Get source data
  df = pd.read_csv("./data/cleaned/patients-clean.csv")
  ...
  scrambled_data.to_csv(
      "./data/cleaned/patients-scrambled.csv",
      index=False,
  )
