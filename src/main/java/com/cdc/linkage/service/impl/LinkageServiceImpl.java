...
import com.cdc.linkage.exceptions.AlgorithmNotCompletedException;
...
public class LinkageServiceImpl implements LinkageService {
  ...
  public Mono<InsertionResponse> recordLinkage(RecordLinkageRequest request) throws Exception {
    ...
    if (!isValid) {
      return Mono.error(new RuntimeException("invalid Data"));
    }
    ...
    return algorithmService.findAlgorithmsByIds(request.algorithmId())
        ...
        .onErrorResume(AlgorithmNotCompletedException.class, ex -> {
  ...
  private Mono<Void> processAlgorithm(Algorithm algorithm) {
    ...
    Mono<Long> algorithmVersionMono = algorithmVersionService.getLastVersionNumber(algorithm.getId());
    return algorithmVersionMono.flatMap(algorithmVersion -> {
      loggerUtil.processAlgorithm(linkageRequest.getId(), algorithm.getId(), algorithmVersion);
      return algorithmService.getAlgorithmDataById(algorithm.getId())
          .flatMap(algorithmData -> {
            checkIFAlgorithmCompleted(algorithmData);
            return getQueryFields(algorithmData.getBlocks(), request.patientRecord())
                .flatMapMany(queryFields -> Flux.fromIterable(queryFields.entrySet()))
                .flatMap(entry -> {
                  return patientService.getPatientsByKeyAndValue(entry.getKey(), entry.getValue())
                      ...
                      .map(patients -> {
                        ...
                        return new AbstractMap.SimpleEntry<>(entry, personIds);
                      });
                })
                .collectList()
                .flatMap(entriesWithPersonIds -> {
                  ...
                  return match(intersectedPersonIds, matchAlgorithm, algorithmData);
                });
          })
          .then();
    });
  ...
  void checkIFAlgorithmCompleted(AlgorithmData algorithmData) {
    if (algorithmData.getBlocks().isEmpty()) {
      throw new AlgorithmNotCompletedException(
          "Algorithm with ID " + algorithmData.getId() + " is not configured properly, because no blocking fields were defined");
    }
    if (algorithmData.getFuncs().isEmpty()) {
      throw new AlgorithmNotCompletedException(
          "Algorithm with ID " + algorithmData.getId() + " is not configured properly, because no criteria fields were defined");
    }
  }
