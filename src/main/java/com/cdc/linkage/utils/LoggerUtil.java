...
public class LoggerUtil {
  ...
  public void fieldRecordsCount(UUID linkageRequestId, String fieldName, String transformation, String fieldValue,
      boolean isFound) {
    fieldValue = fieldValue.replace("%", "").replace("SOUNDEX-", "");//get value only
  ...
  public void processAlgorithm(UUID linkageRequestId, Long algorithmId,Long algorithmVersion) {
    String message = String.format(PROCESS_ALGORITHM, linkageRequestId.toString(), algorithmId,algorithmVersion);
    log.info(message);
    loggingService.saveLog(new LogItem(linkageRequestId, message)).subscribe();
  }
  ...
  public void compareLogOddsThreshold(UUID linkageRequestId, String field, Double threshold, String fieldRequestValue,
      String fieldDatabaseValue,
      Double score, Double fieldLogOdds, UUID personId, UUID patientId) {
    ...
    String message = String.format(COMPARE_LOG_ODDS_THRESHOLD,
        linkageRequestId,
        field,
        threshold,
        fieldValues,
    ...
    String simplifiedMessage = String.format(SIMPLIFIED_COMPARE_LOG_ODDS,
        linkageRequestId,
        personId,
        patientId,
        field,
    ...
    log.info(message);
    log.info(simplifiedMessage);
    loggingService.saveLog(new LogItem(linkageRequestId, message)).subscribe();
    loggingService.saveLog(new LogItem(linkageRequestId, simplifiedMessage)).subscribe();
  }

  public void compareLogOdds(UUID linkageRequestId, String field, String fieldRequestValue,
      String fieldDatabaseValue,
      Double score, UUID personId, UUID patientId) {
  ...
  public void evalLogOddsCutoff(UUID linkageRequestId, String message) {
    log.info(message);
    loggingService.saveLog(new LogItem(linkageRequestId, message)).subscribe();
  }
